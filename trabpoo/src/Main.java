...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Receber gan = new Receber();
        DataReceber dataReceber = new DataReceber();
        DataGastar dataGastos = new DataGastar();
        RelatorioGastos relGastos = new RelatorioGastos();
        RelatorioGanhos relReceber = new RelatorioGanhos();
        ...
        while(true) {
            ...
            if (resposta == 1) {
                ...
                dataGastos.setDia(dia);
                dataGastos.setMes(mes);
                dataGastos.setAno(ano);
                dataGastos.setFormData(dia,mes,ano, count_gasto);
                ...
                System.out.printf("\n %30s: %15s","Data",dataGastos.getFormData(count_gasto));
                ...
                dataReceber.setDia(dia);
                dataReceber.setMes(mes);
                dataReceber.setAno(ano);
                dataReceber.setFormData(dia,mes,ano, count_ganho);
                ...
                System.out.printf("\n %30s: %10s","Data",dataReceber.getFormData(count_ganho));
                System.out.printf("\n %30s: %10.2f\n\n\n\n","Valor",valorGanho);
                ...
                if (resposta == 3) {
                    ...
                    for (i = 0; i < count_gasto; ++i) {
                        ...
                        relGastos.setRelatorio(gas.getNomeGasto(i), gas.getTipoGasto(i), dataGastos.getFormData(i), gas.getValorGasto(i), pagamento, i);
                    ...
                    for (i = 0; i < count_ganho; ++i) {
                        relReceber.setRelatorio(gan.getNomeGanho(i), gan.getTipoGanho(i), dataReceber.getFormData(i), gan.getValorGanho(i), i);
                        relReceber.getRelatorio(i);
                    }
                } else if (resposta == 5) {
                    ...
                    System.out.printf("%30s%15s%15s%15s%25s \n", "Gastos", "Tipo", "Data", "Valor", "Forma de Pagamento");
                    for (i = 0; i < count_gasto; ++i) {
                        if (dataGastos.getFormData(i).contains("/" + mensal + "/" + ano)) {
                            if (gas.getPagamentoGasto(i) == 1) {
                                pagamento = "PIX";
                            } else if (gas.getPagamentoGasto(i) == 2) {
                                pagamento = "Boleto";
                    ...
                    for (i = 0; i < count_ganho; ++i) {
                        if (dataReceber.getFormData(i).contains("/" + mensal + "/" + ano)) {
                            relReceber.setRelatorio(gan.getNomeGanho(i), gan.getTipoGanho(i), dataGastos.getFormData(i), gan.getValorGanho(i), i);
                            relReceber.getRelatorio(i);
    ...
    private static void relGastosgetRelatorio(int i) {
    }

    private static void dataGastossetFormData(int dia, int mes, int ano, int count_gasto) {
    }
}